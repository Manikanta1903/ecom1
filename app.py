...
with mysql.connector.connect(host=host,password=password,db=db,user=user) as conn:
    cursor=conn.cursor()
    cursor.execute("CREATE TABLE if not exists admincreate(email varchar(50) NOT NULL,username varchar(100) NOT NULL,password varbinary(10) NOT NULL,address text NOT NULL,accept enum('on','off') DEFAULT NULL,dp_image varchar(50) DEFAULT NULL,ph_no bigint DEFAULT NULL,PRIMARY KEY(email)")
...
def additem():
    if session.get('admin'):
        if request.method=='POST':
            ...
            # print(filename)
            drname=os.path.dirname(os.path.abspath(_file_))       #
            print(drname)
        ...
        try:
            ...
            if request.method=='POST':
                ...
                if dp_img.filename=='':
                    ...
                    drname=os.path.dirname(os.path.abspath(_file_)) #D:\CODE GNAN\FLASK\Ecommerce
                    static_path=os.path.join(drname,'static')
